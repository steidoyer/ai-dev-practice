...
**상호배타 상태: 겹침(override) vs 분할(split)**
- 같은 속성을 모바일=A / 데스크탑=B로 나눌 때, `A md:B`(겹침)도 정상 동작한다 — **크로스 브레이크포인트 override는 결정적**이다. Tailwind가 변형을 정렬해 무접두(base)를 먼저, `md:`를 뒤에 출력하므로 md 이상에서 `md:`가 base를 **항상** 이긴다(모바일 우선의 정상 관용구). className에 적은 순서와 무관.
- `max-md:A md:B`(분할)로 쓰면 두 규칙의 **적용 구간이 아예 겹치지 않아** "모바일=A / 데스크탑=B" 의도가 자기설명적이 된다. 결과는 겹침과 같지만 **읽기 명확**. (트레이드오프: `max-md:`는 프로젝트 모바일 우선 규칙상 예외 변형이라 남발 금지.)
- ⚠️ **진짜 조심할 겹침은 "같은 변형 레벨 + 같은 속성"** — 예: `bg-accent bg-transparent`(둘 다 무접두). 이땐 승자가 className 순서가 아니라 **Tailwind의 유틸 출력 순서**로 정해져 예측이 어렵다. 배타 조건(삼항 등)으로 **애초에 하나만** 나오게 하는 게 정석. (사례: `CODE_REVIEW_LOG.md` — Journey 와이프 `color` / 예전 배경 유틸.)
