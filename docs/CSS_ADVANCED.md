...
**배경**: Approach 메인 카피 "나는 **융합형** 개발을 지향합니다"에서, 스크롤 진행에 맞춰 "융합형"에 액센트 색이 **왼→오로 쓸고 지나가는** 하이라이트를 넣기로 함(`SECTION_APPROACH_PLAN.md` §3.3). *(Journey 섹션도 마지막 "융합형" 노드에 이 §5 와이프를 재사용 — 처음엔 §5.7 `color-mix` 페이드로 넣었다가 공간 와이프로 변경. `SECTION_JOURNEY_PLAN.md` §3.4.)* 확정: **방향 왼→오 / 색 강조는 메인 카피 "융합형"에만**(원 라벨은 opacity만). 처음엔 `color`를 스크롤로 보간했는데 **"글자가 확 변하는"** 문제가 나와, 그 원인과 대안을 정리한다.
...
- ⚠️ **스톱 위치엔 단위가 필수.** `--wipe`를 `0%~100%`(문자열)로 구동하면 위 CSS 그대로 쓰고, **숫자 `0~1`**로 구동하면(`useTransform(p, range, [0,1])`) 스톱을 `calc(var(--wipe) * 100%)`로 **감싸야** 한다. 단위 없는 `0.5`·`1`은 **유효하지 않은 스톱 위치**라 그 스톱이 통째로 무시되고 와이프가 정지한다(`0`만 예외로 통과해 더 헷갈림). §5.7 `--mix`가 `calc(var(--mix)*100%)`를 쓰는 것과 같은 이유. (사례: `CODE_REVIEW_LOG.md` 2026-09-08.)
